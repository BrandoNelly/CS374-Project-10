...
void new_process(int proc_num, int page_count)
{
  ...
  mem[get_address(0, free_page)] = 1;
  int page_table_page = free_page;
  ...
  for (int i = 0; i < page_count; i++) {
    ...
    mem[get_address(0, free_page)] = 1;
    int page_table_addr = get_address(page_table_page, i);
    mem[page_table_addr] = free_page;
...
void kill_process(int proc_num) {
    ...
    unsigned char page_table_page = get_page_table(proc_num);
    ...
    free_page(page_table_page);
    free_page(get_address(0, PTP_OFFSET + proc_num));
}


int convert_page(int proc_num, int vaddr) {
    unsigned char page_table_page = get_page_table(proc_num);
...
void store_value(int proc_num, int vaddr, int val) {
    int addr = convert_page(proc_num, vaddr);
